#include <stdio.h>
#include <stdlib.h>
#include <curses.h>  // 文本界面的屏幕绘图库
#include <time.h>
#include <unistd.h>
#include <signal.h>

// 游戏主界面 4*4的16宫格，使用二维数组进行表示， 0表示空格
int a[4][4] = {0};
// 16宫格中空格的个数
int empty;
int old_y, old_x;
...
void draw()
{
    ...
    clear();
    for (n = 0; n < 9; n += 2) // 绘制横线， m代表行， n代表列
        for (m = 0; m < 21; m++) {
            move(n, m);
            addch('-');
            refresh();
